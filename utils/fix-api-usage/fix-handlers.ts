...
type APIFixRow = APIFix & RowDataPacket;
const sql = `
...
export async function loadAPIFixData(id: number):Promise<APIFix[]> {
    try {
        const [rows] = await mysql2Pool.query<APIFixRow[]>(sql, {id});
        return rows.map(row => {
...
export async function updateAPIUsage(id: number) {
    try {
